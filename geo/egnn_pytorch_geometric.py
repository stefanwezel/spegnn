...
class Baseline_EGNN_Sparse(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(Baseline_EGNN_Sparse, self).__init__(**kwargs)
        # model params
    ...
    def __repr__(self):
        dict_print = {}
        return "Baseline E(n)-GNN Layer for Graphs " + str(self.__dict__)





class Baseline_EGNN_Sparse_Network(nn.Module):
    ...
    def __init__(self, n_layers, feats_dim, 
        ...
        for i in range(n_layers):
            layer = Baseline_EGNN_Sparse(feats_dim = feats_dim,
                                pos_dim = pos_dim,
    ...
    def forward(self, x, edge_index, batch, edge_attr,
        ...
        # transform inputs 
        coors = x[:, :self.pos_dim]
        # orients = x[:, self.pos_dim:self.pos_dim+1]
        # orients_sin_features = torch.sin(torch.deg2rad(orients))
        # orients_cos_features = torch.cos(torch.deg2rad(orients))
        orients = x[:, self.pos_dim:self.pos_dim+1]
        orients = torch.deg2rad(orients)
        orients_sin_features = torch.sin(orients)
        orients_cos_features = torch.cos(orients)

        feats = x[:, self.pos_dim+1:]
        x = torch.cat([coors, orients_sin_features, orients_cos_features, feats], dim=-1)
